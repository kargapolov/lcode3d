...
class BeamParticles:
    ...
    def save(self, *args, **kwargs):
        particle_type = np.dtype([('xi', 'f8'), ('x', 'f8'), ('y', 'f8'), ('p_x', 'f8'), ('p_y', 'f8'),
                                  ('p_z', 'f8'), ('q_m', 'f8'), ('q_norm', 'f8'), ('id', 'i8')])
...
def particles_weights(x, y, dxi, grid_step_size):  # dxi = (xi_prev - xi)/D_XIP
    i = np.floor(x / grid_step_size).astype(np.int_)
    j = np.floor(y / grid_step_size).astype(np.int_)
    dx = x / grid_step_size - i 
    dy = y / grid_step_size - j
    a0_xi = dxi
    a1_xi = (1 - dxi)
    a0_x = dx
    a1_x = (1 - dx)
    ...
    return i+320, j+320, a000, a001, a010, a011, a100, a101, a110, a111


@nb.njit(cache=True)
def single_particle_weights(x, y, dxi, grid_step_size):  # dxi = (xi_prev - xi)/D_XIP
    i = int(np.floor(x / grid_step_size))
    j = int(np.floor(y / grid_step_size))
    dx = x / grid_step_size - i
    dy = y / grid_step_size - j
    a0_xi = dxi
    a1_xi = (1 - dxi)
...
@nb.njit(cache=True)
def deposit_particles(value, out0, out1, i, j, a000, a001, a010, a011, a100, a101, a110, a111):
...
@nb.njit(cache=True)
def interpolate_particle(value0, value1, i, j, a000, a001, a010, a011, a100, a101, a110, a111):
...
#@nb.njit
def particle_fields(r_vec, grid_step_size, xi_step_size, xi_k, fields_k_1, fields_k):
    dxi = (xi_k - r_vec[2]) / xi_step_size
    i, j, a000, a001, a010, a011, a100, a101, a110, a111 =\
        single_particle_weights(r_vec[0], r_vec[1], dxi, grid_step_size)
    e_x = interpolate_particle(fields_k.Ex, fields_k_1.Ex, i, j, a000, a001, a010, a011, a100, a101, a110, a111)
    e_y = interpolate_particle(fields_k.Ey, fields_k_1.Ey, i, j, a000, a001, a010, a011, a100, a101, a110, a111)
    e_z = interpolate_particle(fields_k.Ez, fields_k_1.Ez, i, j, a000, a001, a010, a011, a100, a101, a110, a111)
    b_x = interpolate_particle(fields_k.Bx, fields_k_1.Bx, i, j, a000, a001, a010, a011, a100, a101, a110, a111)
    b_y = interpolate_particle(fields_k.By, fields_k_1.By, i, j, a000, a001, a010, a011, a100, a101, a110, a111)
    b_z = interpolate_particle(fields_k.Bz, fields_k_1.Bz, i, j, a000, a001, a010, a011, a100, a101, a110, a111)
...
#@nb.njit
def try_move_particle(beam, idx, fields_k_1, fields_k, max_radius, r_step, xi_step_size, xi_layer):
    ...
    while beam.remaining_steps[idx] > 0:
        ...
        e_vec, b_vec = particle_fields(r_vec_1_2, r_step, xi_step_size, xi_k, fields_k_1, fields_k)
        p_vec_1_2 = p_vec_0 + np.sign(q_m) * dt / 2 * (e_vec + cross_nb(p_vec_0 / gamma_0, b_vec))
...
class BeamCalculator:
    def __init__(self, config, beam):
        ...
        self.n_cells = config.grid_steps

        self.rho_layout = np.zeros((self.n_cells, self.n_cells), dtype=np.float64)
    ...
    # Layout particles from [xi_{k+2}, xi_{k+1}) and get beam current density for k+1 step
    def layout_beam(self):
        ...
        if len(idxes) != 0:
            ...
            i, j, a000, a001, a010, a011, a100, a101, a110, a111 = \
                particles_weights(x, y, dxi, self.grid_step_size)
            deposit_particles(self.beam.q_norm[idxes], self.rho_layout, rho_layout, i, j,
    ...
    def move_beam(self, fields_k_1, fields_k):
        ...
        for i in idxes:
            try_move_particle(self.beam, i, fields_k_1, fields_k, self.max_radius, self.grid_step_size,
                              self.xi_step_size, self.xi_layer)
    ...
    def start_moving_layer(self):
        ...
        if len(idxes) == 0:
            return
        dt = beam_substepping_step(self.beam.q_m[idxes], self.beam.p_z[idxes], self.substepping_energy)
