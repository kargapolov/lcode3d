...
def calculate_Ex_Ey_Bx_By(config, const, Ex_avg, Ey_avg, Bx_avg, By_avg,
    ...
    Ex_avg, Ey_avg = Ex_avg, Ey_avg
    Bx_avg, By_avg = Bx_avg, By_avg

    beam_ro, ro = beam_ro, ro
    jx, jy, jz = jx, jy, jz
    jx_prev, jy_prev = jx_prev, jy_prev
...
def move_estimate_wo_fields(config,
    ...
    """
    prev_x_offt, prev_y_offt = prev_x_offt, prev_y_offt
    px, py, pz = px, py, pz
...
def make_plasma(steps, cell_size, coarseness=2, fineness=2):
    ...
    coarse_py = np.zeros((Nc, Nc))
    coarse_pz = np.zeros((Nc, Nc))
    coarse_m = np.ones((Nc, Nc)) * ELECTRON_MASS * coarseness**2
    coarse_q = np.ones((Nc, Nc)) * ELECTRON_CHARGE * coarseness**2
...
def deposit(config, ro_initial, x_offt, y_offt, m, q, px, py, pz, virt_params):
    ...
    deposit_kernel(config.grid_steps, config.grid_step_size,
                        virtplasma_smallness_factor,
...
def step(config, const, virt_params, prev, beam_ro):
    ...
    new_state = Arrays(x_offt=x_offt, y_offt=y_offt, px=px, py=py, pz=pz,
...
def init(config):
    ...
    state = Arrays(x_offt=x_offt, y_offt=y_offt, px=px, py=py, pz=pz,
...
def main():
    ...
    for xi_i in range(config.xi_steps):
        ...
        if time_for_diags or last_step:
            diagnostics(state, config, xi_i, Ez_00_history)
